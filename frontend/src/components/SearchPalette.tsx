...
// Recency at a glance, coarser the older it gets: a time for today, "3d ago" for
// this week, a date beyond that. When several notes match, WHEN it was last touched
// is usually what distinguishes the one being looked for.
function editedLabel(iso: string): string {
  const d = new Date(iso);
  const diff = Date.now() - d.getTime();
  const day = diff / 86400000;
  if (day < 1) return `Edited ${d.toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}`;
  if (day < 7) return `Edited ${Math.round(day)}d ago`;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent animClassName="search-anim" className="top-[12%] w-[min(92vw,42rem)] max-w-none [translate:-50%_0] gap-0 overflow-hidden bg-transparent p-0 max-sm:inset-0 max-sm:left-0 max-sm:top-0 max-sm:flex max-sm:h-full max-sm:w-full max-sm:[translate:0_0] max-sm:flex-col max-sm:rounded-none" hideClose>
        ...
        <div ref={listRef} className="search-surface overflow-y-auto overscroll-contain p-2 max-sm:flex-1 max-sm:p-3 sm:max-h-[60vh]">
          ...
          {!query && (
            <p className="p-6 text-center text-sm text-muted-foreground">Type to search your notes.</p>
          )}
          {results.length > 0 && (
            <>
