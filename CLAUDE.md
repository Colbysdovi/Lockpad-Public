...
Images are `ghcr.io/<owner>/lockpad-backend` and `ghcr.io/<owner>/lockpad-frontend`, built and published by CI on `v*` tags and on manual dispatch. One run per release publishes all three tags: `vX.Y.Z`, `latest` and `sha-<short>`. `latest` therefore tracks the newest release, not the newest commit.
